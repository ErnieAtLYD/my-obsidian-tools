...
export async function scrapeUrl<T = unknown>(url: string) {
	let data: T;
	if (process.env.PROXY_HOST) {
		...
		const proxyAgent = new SocksProxyAgent(
			`socks5://${process.env.PROXY_USERNAME ?? ""}:${process.env.PROXY_PASSWORD ?? ""}@${process.env.PROXY_HOST ?? ""}:${process.env.PROXY_PORT ?? ""}`,
		);
		try {
